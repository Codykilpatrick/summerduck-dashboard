...
const DriversComponent = () => {
        ...
        Papa.parse(csvText, {
          ...
          complete: results => {
            ...
            const uniqueDrivers = Array.from(
              new Set(parsedData.map(record => record.Driver))
            ).sort();
  ...
  useEffect(() => {
    ...
    const reactionTimes = driverRaces
      .filter(race => race.Reaction && race.Reaction > 0)
      .map(race => race.Reaction);
    const sixtyFootTimes = driverRaces
      .filter(race => race['60ft'] && race['60ft'] > 0)
      .map(race => race['60ft']);
    const threeThirtyFootTimes = driverRaces
      .filter(race => race['330ft'] && race['330ft'] > 0)
      .map(race => race['330ft']);
    const eighthETTimes = driverRaces
      .filter(race => race['1/8ET'] && race['1/8ET'] > 0)
      .map(race => race['1/8ET']);
    const eighthMPHSpeeds = driverRaces
      .filter(race => race['1/8MPH'] && race['1/8MPH'] > 0)
      .map(race => race['1/8MPH']);
    ...
    const racesByDate = driverRaces.reduce(
      (acc, race) => {
        if (!acc[race.Date]) {
          acc[race.Date] = [];
        }
    ...
    const performanceData = Object.entries(racesByDate)
      .map(([date, races]) => {
        const dateRaceETs = races.filter(r => r['1/8ET'] && r['1/8ET'] > 0).map(r => r['1/8ET']);
        const dateRaceMPHs = races
          .filter(r => r['1/8MPH'] && r['1/8MPH'] > 0)
          .map(r => r['1/8MPH']);
        const dateReactionTimes = races
    ...
    const opponentData = driverRaces.reduce(
      (acc, race) => {
  ...
  const PerformanceRadarChart = () => {
    ...
    const allDrivers = raceData.reduce(
      (acc, race) => {
        if (!acc[race.Driver]) {
          acc[race.Driver] = {
            reaction: [],
